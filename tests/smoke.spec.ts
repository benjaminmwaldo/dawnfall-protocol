...
test('loads the production shell and starts a solo hunt', async ({ page }) => {
  ...
  for (const retiredCharacter of ['bastion', 'rapunsel', 'eira', 'mara', 'zahra']) {
    await expect(page.locator(`[data-character="${retiredCharacter}"]`)).toHaveCount(0)
  }
  ...
  await expect(page.locator('.party-column')).toHaveCSS('overflow-y', 'auto')
  ...
  const pixelCanvas = await page.locator('#game-canvas').evaluate((canvas) => ({
    logicalWidth: (canvas as HTMLCanvasElement).width,
    displayWidth: (canvas as HTMLCanvasElement).clientWidth,
  ...
  await page.mouse.up()
  await expect(page.locator('#ammo-hud .pixel-ammo')).toContainText('∞')
  ...
  for (const art of ['upgrade-vesper-1.webp', 'upgrade-seraph-5.webp', 'upgrade-backdrop-1.webp', 'upgrade-rapunsel-5.webp', 'upgrade-cinder-1.webp', 'upgrade-cinder-2.webp', 'upgrade-cinder-3.webp', 'upgrade-cinder-4.webp', 'upgrade-cinder-5.webp', 'upgrade-bastion-1.webp', 'upgrade-bastion-2.webp', 'upgrade-bastion-3.webp', 'upgrade-bastion-4.webp', 'upgrade-bastion-5.webp', 'upgrade-warden-1.webp', 'upgrade-warden-2.webp', 'upgrade-warden-3.webp', 'upgrade-warden-4.webp', 'upgrade-warden-5.webp', 'cinder-portrait.webp', 'ama-portrait.webp', 'rapunsel-portrait.webp', 'aiko-portrait.webp', 'eira-portrait.webp', 'mara-portrait.webp', 'zahra-portrait.webp', 'recap-victory-1.webp', 'recap-defeat-5.webp']) {
